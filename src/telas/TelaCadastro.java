...
public class TelaCadastro extends JDialog implements ActionListener, KeyListener {
	...
	// Construtor 
	public TelaCadastro(Class<?> classe) {
	...
	public void init(Class<?> classe) {
		myFont = RegistrarFont.minhaFont(PixelFont.PixelOperator, "30f");
		corFundo = Cores.corCinza;
		corVerde = Cores.corVerde;
		corVermelha = Cores.corVermelha;
		...
		// Label Aviso
		lblAviso = new MyLabel(500, 50, "Você tem que digitar mais de 3 caracteres", myFont, corVermelha, null);
	...
	public void vefNome() {
		...
		if (txfNome.getText().length() < 3) {
			...
			CRUDUsuario insert = new CRUDUsuario();
			insert.insereUsuario(txfNome.getText().trim(), null, null);

			TelaCarregamento.getInstance(TelaPrincipal.class);
	...
	public void keyTyped(KeyEvent e) {
		// Não permitir que ele digite mais do que 25 caracteres
