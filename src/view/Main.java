...
public class Main extends JFrame {
    ...
    // Musica 
 	protected static Sound objSound;
	...
	private void init() {
		// JPanels
		tela = new MyPanel();
		areaQuestao = new MyPanel(d.width, 500, Colors.corCinza);
		...
		objSound = new Sound();
		musicaFundo = statics.Sound.goblinSlayerOP;
		
		myFont = RegisterFont.minhaFont(PixelFont.PixelOperator, "25f");
		
		principal = new Character();
		inimigo = new Character();
		...
		// Principal
		adventure = new InfiniteAnimation("adventure-idle", 150, 88, 102, false);
		adventure.setLocation(150, batalha.getHeight() - adventure.getHeight() - 50);
	
		// Inimigo
		mob = new InfiniteAnimation("skeleton-idle", 75, 84, 119, true);
		...
		//Progress Bar
		pbTempoResposta = new MyProgressBar(0, 100, areaQuestao.getWidth()/100*50, 20, Colors.corVerde, Color.BLACK, 3);
		...
		for (int i = 0; i < 4; i++) {
			btnAlternativa[i] = new MyButton (
				...
				myFont, LabelHTML.html(game.getQuestions().get(indexQuestao).getAlternatives().get(i).getTexto(), "50"),
				Color.BLACK, 3,
	...
	public void atualizarQuestao() {
		if(indexQuestao + 1 < game.getQuestions().size()) {
			...
			for(int i = 0; i < 4; i++) {
				...
				btnAlternativa[i].setText(LabelHTML.html(game.getQuestions().get(indexQuestao).getAlternatives().get(i).getTexto(), "50"));
			}
			
			loading = new Thread(new CarregarBarra());
			loading.start();
			pbTempoResposta.setForeground(Colors.corVerde);
	...
	public class CarregarBarra implements Runnable {
		...
		public void run() {
			for (int i = 100; i >= 0; i--) {
				try {
					...
					if (pbTempoResposta.getValue() <= 25) {
						pbTempoResposta.setForeground(Colors.corVermelha);
						
					} else if (pbTempoResposta.getValue() <= 50) {
						pbTempoResposta.setForeground(Colors.corLaranja);
						
					} else if (pbTempoResposta.getValue() <= 75) {
						pbTempoResposta.setForeground(Colors.corAmarela);
			...
			for(int j = 0; j < 4; j++) {
				if(game.getQuestions().get(indexQuestao).getAlternatives().get(j).isCorreta()) {
					btnAlternativa[j].setBackground(Colors.corVerde);
				}
			}
	...
	public void btnAlternativa(int i, int k) {
		btnAlternativa[i].addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				// Se o jogo est� ativo
				if(game.isAtivo()) {
					// Se a alternativa escolhida est� correta
					if(game.getQuestions().get(indexQuestao).getAlternatives().get(k).isCorreta()) {
						btnAlternativa[k].setBackground(Colors.corVerde);
						game.setAcertos();
						inimigo.setVidas();
						ataque(true);
						...
						btnAlternativa[k].setBackground(Colors.corVermelha);
						
						for(int j = 0; j < 4; j++) {
							// Marcar a op��o correta de verde
							if(game.getQuestions().get(indexQuestao).getAlternatives().get(j).isCorreta()) {
								btnAlternativa[j].setBackground(Colors.corVerde);
